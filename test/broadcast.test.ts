...
test('broadcast: a public announcement round-trips through the message record + public envelope', () => {
...
test('broadcast: openPublicEnvelope refuses an ENCRYPTED envelope (those stay private DMs)', () => {
  ...
  assert.equal(openPublicEnvelope(envelope), null)
  // …but the intended recipient still opens it the encrypted way.
  assert.equal(openEnvelope(envelope, recipient)?.parts.find(isText)?.text, 'secret')
})
