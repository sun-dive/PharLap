...
const PBPS = 250        // 2.5%
const PUBLISHER_CUT = Math.floor((PRICE * PBPS) / 10000) // 2500
const RESELLER_CUT = PRICE - PUBLISHER_CUT               // 97500

// Lock layout mirrors the real edition's pubkey→price adjacency: <holderPub><price(8)> OP_2DROP <v2 branch>.
function buildV2Lock(F: number, holderPub: number[], publisherHash: number[]) {
  return new LockingScript([
    { op: holderPub.length, data: holderPub },
    { op: 8, data: u64le(PRICE) },
    op(OP.OP_2DROP),
    ...replicateBranchV2Ops({ fieldPubkeyOffset: F, tokenSats: 1, publisherPubKeyHash: publisherHash, pBps: PBPS, c: pushTxConstants() }),
  ])
}
function v2FieldOffset(holderPub: number[], publisherHash: number[]): number {
  const len = buildV2Lock(0, holderPub, publisherHash).toBinary().length
  return (len < 253 ? 1 : len < 65536 ? 3 : 5) + PUBKEY_OFFSET
}

interface Override { publisherFee?: number; resellerFee?: number; out1Pub?: number[]; publisherHash?: number[] }
function runReplicateV2(ov: Override = {}): boolean {
  ...
  const lock = buildV2Lock(v2FieldOffset(holderPub, publisherHash), holderPub, publisherHash)
  ...
  sp.addOutput({ lockingScript: LockingScript.fromBinary(publisherScript), satoshis: ov.publisherFee ?? PUBLISHER_CUT })
...
test('v2 replicate: enforces the computed percentage split (publisher ⌊P·c%⌋, reseller remainder)', () => {
  assert.equal(runReplicateV2(), true)
})

test('v2 replicate: rejects short-paying the computed publisher cut', () => {
  assert.throws(() => runReplicateV2({ publisherFee: PUBLISHER_CUT - 1 }))
})

test('v2 replicate: rejects over-paying the reseller (must equal P − publisherCut exactly)', () => {
  assert.throws(() => runReplicateV2({ resellerFee: RESELLER_CUT + 1 }))
})

test('v2 replicate: rejects redirecting the publisher cut to another address', () => {
  const attacker = Hash.hash160(PrivateKey.fromRandom().toPublicKey().encode(true) as number[])
  assert.throws(() => runReplicateV2({ publisherHash: attacker }))
})
...
function runEditionV2Replicate(ov: { publisherFee?: number } = {}): boolean {
  ...
  const lock = buildEditionLockV2({
    ...
    publisherPubKeyHash: publisherHash, pBps: PBPS, tokenSats: 1,
  })
  const lockBytes = lock.toBinary()
  ...
  sp.addOutput({ lockingScript: LockingScript.fromBinary(out1), satoshis: 1 })
  sp.addOutput({ lockingScript: LockingScript.fromBinary(p2pkhScript(publisherHash)), satoshis: ov.publisherFee ?? PUBLISHER_CUT })
...
test('v2 FULL edition lock: rejects short-paying the publisher cut', () => {
  assert.throws(() => runEditionV2Replicate({ publisherFee: PUBLISHER_CUT - 1 }))
})

test('parseEditionScriptV2 recovers owner, price, publisher hash + pBps; v1 parser rejects a v2 lock', () => {
  const holderPub = PrivateKey.fromRandom().toPublicKey().encode(true) as number[]
  const publisherHash = Hash.hash160(PrivateKey.fromRandom().toPublicKey().encode(true) as number[])
  ...
  assert.equal(parsed!.terms.pBps, PBPS)
