...
export async function postToNodeFeed(
  ...
  const selected = selectFunding(await getSafeUtxos(provider), PHARLAP_OUTPUT_SATS + 1 + 700)
  ...
  tx.addOutput({ lockingScript: LockingScript.fromBinary(p2pkhScript(params.feedHash160)), satoshis: 1 })                   // [1] feed breadcrumb
  tx.addOutput({ lockingScript: new P2PKH().lock(key.toAddress()), change: true })                                          // [2] change
  ...
  provider.registerPendingTx(txId, selected.map(u => ({ txId: u.txId, outputIndex: u.outputIndex })),
    (tx.outputs[2]?.satoshis ?? 0) > 0 ? { outputIndex: 2, satoshis: tx.outputs[2].satoshis ?? 0 } : undefined)
  return txId
}

/** Scan one node's feed (by its derived feed address) and return its posts. A post is authentic iff its
 *  RECORD_MESSAGE is self-locked (envelope sender == lock key) and keyed to this node's `ref`. */
export async function scanNodeFeed(provider: WalletProvider, feedHash160: number[], ref: string): Promise<DiscPost[]> {
...
/** A node in a holder's corridor, with its feed identity resolved (for read + post). */
export interface CorridorNode extends DiscNode { feedHash160: number[]; ref: string; isRoot: boolean; isSelf: boolean }
...
export async function resolveCorridor(
  ...
  const out: CorridorNode[] = [{
    ...
    feedHash160: rootFeedHash160(collectionId), ref: collectionId, isRoot: true, isSelf: false,
  }]
...
/** Read a holder's whole corridor feed for a collection: every node's posts, merged newest-first, each tagged
 *  with the node it came from. (Upstream + own; downstream-of-you arrives once breadcrumbs ship.) */
export async function readCorridor(
  ...
  const posts: Array<DiscPost & { node: CorridorNode }> = []
  for (const node of nodes) {
    const feed = await scanNodeFeed(provider, node.feedHash160, node.ref)
    for (const p of feed) posts.push({ ...p, node })
  }
  posts.sort((a, b) => (b.sentAt ?? 0) - (a.sentAt ?? 0)) // newest first
  return { nodes, posts }
}
